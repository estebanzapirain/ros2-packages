...
class Metaclass_Block(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('map_interface')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'map_interface.msg.Block')
            logger.debug(
...
class Block(metaclass=Metaclass_Block):
    ...
    def sig(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'sig' field must be of type 'int'"
            assert value >= -9223372036854775808 and value < 9223372036854775808, \
                "The 'sig' field must be an integer in [-9223372036854775808, 9223372036854775807]"
        self._sig = value
